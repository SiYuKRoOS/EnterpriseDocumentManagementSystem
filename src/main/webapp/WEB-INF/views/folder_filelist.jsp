            ...
            <div class="widget am-cf theme-black" style="border: none; background: none;">
              <div class="widget-head am-cf" style="padding: 0">
                <div class="widget-title am-cf">
                  <c:choose>
                    <c:when test="${from == 'download' }">
                      <c:if test="${parentid>0 }">
                        <a href="<%=request.getContextPath() %>/folder/viewFolder?folderId=${parentid}&from=${from}">&lt;&lt;上一级 </a>
                      </c:if>
                      <c:if test="${parentid==0 }">
                        <a href="<%=request.getContextPath()%>/file/downloadlist">&lt;&lt;上一级 </a>
                      </c:if>
                    ...
                    <c:otherwise>
                      ...
                      <c:if test="${parentid==0 }">
                        <a href="<%=request.getContextPath()%>/file/uploadlist">&lt;&lt;上一级 </a>
                      </c:if>
					  |
					    当前目录：/${folder.remark }
					  <div class="am-form-group am-fr">
			 ...
			 $('#addFolderBtn').on('click', function() {
			    $('#my-prompt').modal({
				      ...
				      onConfirm: function(e) {
				    	...
				    	var hiddenFolderId = $("#hiddenFolderId").val();
				        $.ajax({
							url : "<%=request.getContextPath()%>/folder/createFolder?from=${from}",
							type : 'POST',
							data : {folderName:e.data, hiddenFolderId:hiddenFolderId},
							success : function(data) {
								alert("新建成功...")
								window.location.reload();
								return ;
